...
class NCADish:
    ...
    def seed(self, seeds: np.array):
        ...
        self.seeds[:, : seeds.shape[1], : seeds.shape[2], : seeds.shape[3]] = seeds
    ...
    def grow(
        ...
        optimizer = torch.optim.Adam(self.model.parameters(), lr=learnign_rate)
        ...
        for epoch in (pbar := tqdm(range(epochs))):
            ...
            batch_idxs = self._random_sample_pool(basic_seeds_pool, batch_size)
    ...
    def _prepare_pool(self, source):
        if source is not None:
            pool = source.repeat(self.pool_size, 1, 1, 1).to(self.device).clone()
        else:
            pool = np.array([None] * self.pool_size)
        return pool

    def _random_sample_pool(self, pool, batch_size):
        batch_idxs = np.random.choice(pool.shape[0], batch_size, replace=False).tolist()
        return batch_idxs
