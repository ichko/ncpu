...
#
# Usage:
#   bash scripts/other/run_extrapolation_sweep.sh
#   STEPS=50000 bash scripts/other/run_extrapolation_sweep.sh   # faster smoke test
...
for k in "${K_VALUES[@]}"; do
    for seed in "${SEEDS[@]}"; do
        ...
        echo "────────────────────────────────────────────────────"
        uv run python scripts/train/train_extrapolation.py \
            --max_train_bits "$k" \
            --seed "$seed" \
            --steps "$STEPS"
    done
...
echo "  uv run python scripts/analyze/analyze_extrapolation.py"
