...
filmstrip PNGs and embeddings that this script regenerates from the training runs:
    ...
    demo/data/rollouts/*.png + subleq_fib.json   scripts/other/capture_subleq_demo.py
    demo/data/carry_*                            scripts/other/bake_carry_tsne.py
    movement clusters inside carry_tsne.json     scripts/other/cluster_carry_cells.py
...
checkpoint first and reported as SKIPPED rather than failing the whole build.
    ...
    uv run python scripts/other/build_demo.py                # build what is missing
    uv run python scripts/other/build_demo.py --force        # rebuild everything
    uv run python scripts/other/build_demo.py --only carry   # one target
    uv run python scripts/other/build_demo.py --deploy       # build, then Cloudflare Pages
