...
Usage:
    uv run python scripts/train_gate.py --gate XOR --seed 0
    uv run python scripts/train_gate.py --gate half_adder --seed 1
...
is_ablation    = args.kernel_size != 5 or args.alive_threshold != 0.0
experiment     = "E1" if not is_ablation else ("E_KS" if args.kernel_size != 5 else "E_AM")
...
run_name       = f"{experiment}_{args.gate}{kernel_tag}{alive_tag}_s{args.seed}_{timestamp}"
...
with open(run_dir / "config.json", "w") as f:
    json.dump({
        "run":   {"name": run_name, "experiment": experiment, "gate": args.gate,
