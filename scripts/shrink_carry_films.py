...
alone and reported. Nothing here touches the embeddings, so it needs no re-bake.
    ...
    uv run python scripts/shrink_carry_films.py            # report only
    uv run python scripts/shrink_carry_films.py --apply     # rewrite the files
"""
import argparse
...
if __name__ == "__main__":
    ...
    if not films:
        raise SystemExit("no carry filmstrips in demo/data — run scripts/bake_carry_tsne.py first")
